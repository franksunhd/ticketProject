  ...
  <Box>
    ...
    <div class="tableWp">
      ...
      >
        ...
        <el-table-column fixed prop="ORDER_ID" label="上交单号" width="200" align="center" />
        ...
        <el-table-column prop="ORDER_STATE_NAME" label="上交单状态" align="center" sortable />
    ...
    <div class="dialog">
      ...
      >
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              ...
              <el-form-item label="创建位置：">
                <el-select v-model="addEdit.createLocation" class="widthAll" disabled>
                  <el-option
                    v-for="(item,index) in stationList"
                    :key="index"
                    :label="item.STATION_LOCATION_NAME"
              ...
              <el-form-item label="调出位置：">
                <el-select v-model="addEdit.outLocation" class="widthAll" disabled>
                  <el-option
                    v-for="(item,index) in stationList"
                    :key="index"
                    :label="item.STATION_LOCATION_NAME"
        ...
        <div class="lowerPart">
          <el-table border :data="addList" stripe>
            ...
            <el-table-column label="上交数量(张)" align="center">
              <template slot-scope="scope">
                <el-input
      ...
      >
        <div slot="title" class="dialogTitle clearFix">
          <span class="title">添加上交单票卡类型</span>
      ...
      >
        ...
        <el-table border :data="ticketTypeNum" stripe>
          ...
          <el-table-column prop="requestNum" label="申请上交数量(张)" align="center" />
          <el-table-column label="实际上交数量(张)" align="center">
            <template slot-scope="scope">
              <el-input
      ...
      >
        ...
        <el-table border :data="tableDetail" stripe>
          ...
          <el-table-column prop="QUANTITY_REQUEST" label="计划上交数量(张)" align="center" />
          <el-table-column prop="QUANTITY_ACTUAL" label="实际上交数量(张)" align="center" />
  ...
  data() {
    return {
      ...
      addEdit: {
        ...
        type: "03", //单据类型   01配发 02调拨 03上交 04回收
        commentInfo: "" //备注
      },
      // 创建表单校验
      errorTip: {
  ...
  methods: {
    ...
    getData() {
      ...
      if (location_number !== "" && user_id !== null && user_id !== "null") {
        ...
        let postData = {
          ...
          order_type: "03", //单据类型 01配发 02调拨 03上交 04回收
          ...
          if (
            ...
            if (
              ...
              var params = {
                ...
                comment_info: _t.addEdit.commentInfo.trim()
              };
              _t.$api.post(
    ...
    ticketCommit() {
      ...
      if (_t.ticketTypeNum.length !== 0) {
        ...
        if (isErrorNum !== 0) {
          // 有错误
          _t.alertDialog("请检查实际上交数量是否为空或者是否大于库存数据!");
        } else {
          // 没有错误 可以提交
          if (
            _t.moneyFormItem.userFlag === false &&
    ...
    enterStation(linesStationArr) {
      ...
      if (
        ...
        // 选中的车站
        if (LOCATION_NUMBER !== "") {
          _t.addEdit.inLocation = LOCATION_NUMBER;
        }
      }
    },
    // 处理线路及车站首次加载数据问题
  ...
  created() {
    ...
    if (limitMenuList !== null && limitMenuList.length !== 0) {
      ...
      limitBtnArr.forEach(item => {
        // 查询
        if (item.MENU_ID === "GUID00000000000000000000000000000038") {
          _t.limitBtn.queryBtn = true;
        }
        // 创建上交单
        if (item.MENU_ID === "GUID00000000000000000000000000000039") {
        ...
        // 撤销
        if (item.MENU_ID === "GUID00000000000000000000000000000133") {
          _t.limitBtn.undo = true;
        }
        // 接收
        if (item.MENU_ID === "GUID00000000000000000000000000000040") {
          _t.limitBtn.accept = true;
        }
        // 拒绝
        if (item.MENU_ID === "GUID00000000000000000000000000000041") {
          _t.limitBtn.refuse = true;
        }
        // 备票
        if (item.MENU_ID === "GUID00000000000000000000000000000042") {
          _t.limitBtn.ticket = true;
        }
        // 配送
        if (item.MENU_ID === "GUID00000000000000000000000000000043") {
          _t.limitBtn.distribution = true;
        }
        // 上交单详情
        if (item.MENU_ID === "GUID00000000000000000000000000000044") {
