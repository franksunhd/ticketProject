  ...
  <Box>
    ...
    <div class="tableWp">
      ...
      >
        ...
        <el-table-column fixed prop="ORDER_ID" label="回收单号" width="200" align="center" />
        ...
        <el-table-column prop="ORDER_STATE_NAME" label="回收单状态" align="center" sortable />
    ...
    <div class="dialog">
      ...
      >
        ...
        <div class="lowerPart">
          <el-table border :data="addList" stripe>
            ...
            <el-table-column label="回收数量(张)" align="center">
              <template slot-scope="scope">
                <el-input
      ...
      >
        <div slot="title" class="dialogTitle clearFix">
          <span class="title">添加回收单票卡类型</span>
      ...
      >
        ...
        <el-table border :data="ticketTypeNum" stripe>
          ...
          <el-table-column prop="requestNum" label="申请回收数量(张)" align="center" />
          <el-table-column label="实际回收数量(张)" align="center">
            <template slot-scope="scope">
              <el-input
      ...
      >
        ...
        <el-table border :data="tableDetail" stripe>
          ...
          <el-table-column prop="QUANTITY_REQUEST" label="计划回收数量(张)" align="center" />
          <el-table-column prop="QUANTITY_ACTUAL" label="实际回收数量(张)" align="center" />
  ...
  data() {
    return {
      ...
      addEdit: {
        ...
        type: "04", //单据类型  01配发 02调拨 03上交 04回收
        commentInfo: "" //备注
      },
      // 创建回收单校验
      errorTip: {
  ...
  methods: {
    ...
    getData() {
      ...
      if (location_number !== "" && user_id !== null && user_id !== "null") {
        ...
        let postData = {
          ...
          order_type: "04", //单据类型 01配发 02调拨 03上交 04回收
    ...
    addEditData() {
      ...
      if (_t.addList.length === 0) {
        ...
        if (_t.addEdit.inLocation === _t.addEdit.outLocation) {
          _t.alertDialogTip(_t, "调出位置和调入位置不能一样!");
        } else {
          if (isErrorNumber !== 0) {
            _t.alertDialogTip(
              _t,
              "请检查回收数量是否为空、格式是否正确、且不能大于库存数量!"
            );
          } else {
            // 可以提交
            if (
    ...
    ticketCommit() {
      ...
      if (_t.ticketTypeNum.length !== 0) {
        ...
        if (isErrorNum !== 0) {
          // 有错误
          _t.alertDialog("请检查实际回收数量是否为空或者是否大于库存数据!");
        } else {
          // 没有错误 可以提交
          if (
            _t.moneyFormItem.userFlag === false &&
  ...
  created() {
    ...
    if (limitMenuList !== null && limitMenuList.length !== 0) {
      ...
      limitBtnArr.forEach(item => {
        // 查询
        if (item.MENU_ID === "GUID00000000000000000000000000000120") {
          _t.limitBtn.queryBtn = true;
        }
        // 创建回收单
        if (item.MENU_ID === "GUID00000000000000000000000000000121") {
          ...
          if (
            ...
            // MLC角色 location_number MLC 启用创建按钮
            if (roleName === "MLC" && locationNum.substr(0, 1) === "1") {
              // 取消禁用
        ...
        // 撤销
        if (item.MENU_ID === "GUID00000000000000000000000000000122") {
          _t.limitBtn.undo = true;
        }
        //拒绝
        if (item.MENU_ID === "GUID00000000000000000000000000000132") {
          _t.limitBtn.refuse = true;
        }
        // 接收
        if (item.MENU_ID === "GUID00000000000000000000000000000123") {
          _t.limitBtn.accept = true;
        }
        // 备票
        if (item.MENU_ID === "GUID00000000000000000000000000000124") {
          _t.limitBtn.ticket = true;
        }
        // 配送
        if (item.MENU_ID === "GUID00000000000000000000000000000125") {
          _t.limitBtn.distribution = true;
        }
        // 回收单详情
        if (item.MENU_ID === "GUID00000000000000000000000000000126") {
