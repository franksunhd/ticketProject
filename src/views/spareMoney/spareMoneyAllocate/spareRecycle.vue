  ...
  <Box>
    <div class="formBox-top">
      <el-form inline ref="form" :model="formItem" label-width="120px">
        ...
        <el-form-item label="回收单号:" class="marBottom7">
          <el-input class="width200" v-model="formItem.orderNum" placeholder="请输入回收单号"/>
        </el-form-item>
    ...
    <div class="tableWp">
      ...
      <el-table border stripe :data="tableData" @row-click="rowClick"
        ...
        <el-table-column prop="ORDER_ID" label="回收单号" header-align="center" align="center"/>
        ...
        <el-table-column prop="ORDER_STATE_NAME" sortable label="回收单状态" header-align="center" align="center"/>
        ...
        <el-table-column label="操作" header-align="center" align="center">
          <template slot-scope="scope">
            <el-button v-if="limitBtn.detail" type="primary" @click.stop="detailBtn(scope.row)">回收单详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
      ...
      <el-form :model="addEdit" ref="addEdit" label-width="140px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            ...
            <el-form-item class="star" label="调出位置：">
              <el-select
                v-model="addEdit.outLocation"
                clearable
                @change="changeLocation(addEdit.outLocation)"
                class="widthAll">
                <el-option
              ...
              <span class="isNotNull" v-if="errorTip.outLocationFlag">{{errorTip.outLocationTip}}</span>
            </el-form-item>
      ...
      <el-form :model="moneyFormItem" ref="moneyFormItem" label-width="150px">
        <el-row :gutter="20">
          <el-col :span="12">
            ...
            <el-form-item label="当前库存金额(元)：">
              <el-input v-model="moneyFormItem.allMoney" disabled/>
            </el-form-item>
      ...
      <el-form :model="detailFormItem" ref="detailFormItem" label-width="140px">
        <el-row :gutter="20">
          <el-col :span="12">
            ...
            <el-form-item label="单据类型：">
              <el-input v-model="detailFormItem.orderType === '02' ? '回收单' : ''" disabled/>
            </el-form-item>
            <el-form-item label="调入位置：">
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                isScreenCreate: false, // 创建回收单放大缩小
        ...
        methods: {
                ...
                if (roleName !== null && roleName !== 'null' && location_number !== null && location_number !== 'null') {
                    switch (row.ORDER_STATE) {
                        case '01': // 已申请(MLC撤销、SC接收和拒绝)
                            ...
                            } else if (roleName === 'SC' || (roleName === 'MLC' && location_number.substr(0, 1) === '1')) {
            ...
            getData() {
                ...
                if (location_number !== '' && user_id !== null && user_id !== 'null') {
                    ...
                    var params = {
                        ...
                        order_type: '02', // 01: 配发 02:回收 03:上交
            ...
            changeLocation(val) {
                var _t = this;
                if (val === '') {
                    _t.errorTip.outLocationFlag = true;
                    _t.errorTip.outLocationTip = '必填项不能为空';
                } else {
                    // 不为空 校验调出位置和调入位置
                    if (val === _t.addEdit.inLocation) {
                        _t.errorTip.outLocationFlag = true;
                        _t.errorTip.outLocationTip = '调入位置和调出位置不能相同';
                    } else {
                        _t.errorTip.outLocationFlag = false;
                        _t.errorTip.outLocationTip = '';
                        // 获取车站的库存记录
                        var user_id = _t.getCookie('USER_ID');
                        if (user_id !== null && user_id !== 'null') {
                            _t.getDetailData(_t.addEdit.outLocation, user_id, true); // true 创建
                        }
                    }
                }
            },
            ...
            addData() {
                var _t = this;
                // 校验调出位置
                if (_t.addEdit.outLocation === '') {
                    _t.errorTip.outLocationFlag = true;
                    _t.errorTip.outLocationTip = '必填项不能为空';
                } else {
                    // 不为空 校验调出位置和调入位置
                    if (_t.addEdit.outLocation === _t.addEdit.inLocation) {
                        _t.errorTip.outLocationFlag = true;
                        _t.errorTip.outLocationTip = '调入调出位置不能为空';
                    } else {
                        _t.errorTip.outLocationFlag = false;
                        _t.errorTip.outLocationTip = '';
                    }
                ...
                // 校验操作金额
                _t.blurInputMoney();
                if (_t.errorTip.outLocationFlag === false
                    && _t.errorTip.userFlag === false
                    && _t.errorTip.passwordFlag === false
                    && _t.errorTip.moneyFlag === false) {
            ...
            createBtn() {
                ...
                if (location_number !== null && location_number !== 'null' && user_id !== null && user_id !== 'null') {
                    ...
                    _t.addEdit.inLocation = location_number; // 调入位置
            ...
            resetData() {
                ...
                _t.addEdit.operationType = '备用金回收'; // 操作类型
                _t.addEdit.allMoney = '0'; // 当前库存金额
                _t.addEdit.operationMoney = ''; // 调拨金额
                _t.addEdit.note = ''; // 备注
                _t.errorTip.outLocationFlag = false;// 调入位置
                ...
                _t.errorTip.outLocationTip = ''; // 调出位置提示
            },
            // 撤销
            undoBtn() {
                var _t = this;
            ...
            ticketBtn() {
                ...
                if (location_number !== null && location_number !== 'null' && user_id !== null && user_id !== 'null') {
                    ...
                    _t.getDetailData(location_number, user_id, false); // false 库存详情
                    _t.getSendDetail(location_number, user_id); // 配发金额
                }
            },
            // 获取备款详情
        ...
        created() {
            ...
            if (limitMenuList !== null && limitMenuList.length !== 0) {
                ...
                limitBtnArr.forEach((item) => {
                    // 查询
                    if (item.MENU_ID === "GUID00000000000000000000000000000072") {
                        _t.limitBtn.queryBtn = true;
                    }
                    // 创建回收单
                    if (item.MENU_ID === "GUID00000000000000000000000000000073") {
                    ...
                    // 撤销
                    if (item.MENU_ID === "GUID00000000000000000000000000000074") {
                        _t.limitBtn.undo = true;
                    }
                    // 接收
                    if (item.MENU_ID === "GUID00000000000000000000000000000075") {
                        _t.limitBtn.accept = true;
                    }
                    // 拒绝
                    if (item.MENU_ID === "GUID00000000000000000000000000000076") {
                        _t.limitBtn.refuse = true;
                    }
                    // 备票
                    if (item.MENU_ID === "GUID00000000000000000000000000000077") {
                        _t.limitBtn.ticket = true;
                    }
                    // 配送
                    if (item.MENU_ID === "GUID00000000000000000000000000000078") {
                        _t.limitBtn.distribution = true;
                    }
                    // 回收单详情
                    if (item.MENU_ID === "GUID00000000000000000000000000000079") {
